...
public class QuartzRouteDaily extends RouteBuilder{
	...
	public void configure() throws Exception {
		...
		from("quartz2://everyDayAt_9_30?cron=0+30+10+?+*+MON,TUE,WED,THU,FRI+*").process(downloadNSEBhavProcessor)
		.to("log:everyDayAt_10_30");

		from("quartz2://everyDayAt_16_45?cron=0+50+10+?+*+MON,TUE,WED,THU,FRI+*").process(researchUpdateProcessor)
		.to("log:everyDayAt_16_45");
		
		from("quartz2://everyDayAt_9_45?cron=0+52+10+?+*+MON,TUE,WED,THU,FRI+*").process(emailPortfolioProcessor)
