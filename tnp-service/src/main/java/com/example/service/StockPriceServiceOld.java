...
public class StockPriceServiceOld {
	...
	public boolean isTmaConvergence(Stock stock, Timeframe timeframe, Trend trend) {
		...
		if (timeframe == Timeframe.DAILY){
			return this.isTmaConvergence(stockTechnicals.getEma20(), stockTechnicals.getEma50(), stockTechnicals.getEma200(), stockTechnicals.getPrevEma20(), stockTechnicals.getPrevEma50(), stockTechnicals.getPrevEma200());
		}else if (timeframe == Timeframe.WEEKLY){
			return this.isTmaConvergence(stockTechnicals.getEma10(), stockTechnicals.getEma20(), stockTechnicals.getEma50(), stockTechnicals.getPrevEma10(), stockTechnicals.getPrevEma20(), stockTechnicals.getPrevEma50());
		}else if (timeframe == Timeframe.MONTHLY){
			return this.isTmaConvergence(stockTechnicals.getEma5(), stockTechnicals.getEma10(), stockTechnicals.getEma20(), stockTechnicals.getPrevEma5(), stockTechnicals.getPrevEma10(), stockTechnicals.getPrevEma20());
		}
	...
	private boolean isTmaConvergence(double immediateLow, double average, double immediateHigh, double prevImmediateLow, double prevAverage, double prevImmediateHigh){
	...
	public boolean isCloseAboveEma(Stock stock, Timeframe timeframe){
		...
		StockTechnicals stockTechnicals = stockTechnicalsService.get(stock, timeframe);
		if(timeframe == Timeframe.DAILY) {
			return CrossOverUtil.isFastCrossesAboveSlow(stockPrice.getPrevClose(), stockTechnicals.getPrevEma20(), stockPrice.getClose(), stockTechnicals.getEma20());
		}else if(timeframe == Timeframe.WEEKLY) {
			return CrossOverUtil.isFastCrossesAboveSlow(stockPrice.getPrevClose(), stockTechnicals.getPrevEma10(), stockPrice.getClose(), stockTechnicals.getEma10());
		}if(timeframe == Timeframe.MONTHLY) {
			return CrossOverUtil.isFastCrossesAboveSlow(stockPrice.getPrevClose(), stockTechnicals.getPrevEma5(), stockPrice.getClose(), stockTechnicals.getEma5());
		}
		return false;
	}
	...
	public boolean isCloseBelowEma(Stock stock, Timeframe timeframe){
		...
		StockTechnicals stockTechnicals = stockTechnicalsService.get(stock, timeframe);
		if(timeframe == Timeframe.DAILY) {
			return CrossOverUtil.isSlowCrossesBelowFast(stockPrice.getPrevClose(), stockTechnicals.getPrevEma20(), stockPrice.getClose(), stockTechnicals.getEma20());
		}else if(timeframe == Timeframe.WEEKLY) {
			return CrossOverUtil.isSlowCrossesBelowFast(stockPrice.getPrevClose(), stockTechnicals.getPrevEma10(), stockPrice.getClose(), stockTechnicals.getEma10());
		}if(timeframe == Timeframe.MONTHLY) {
			return CrossOverUtil.isSlowCrossesBelowFast(stockPrice.getPrevClose(), stockTechnicals.getPrevEma5(), stockPrice.getClose(), stockTechnicals.getEma5());
		}
		return false;
	}
