...
public class ResearchInsightService {
    ...
    public boolean isStrongInsights(Stock stock) {
        ...
        try {
            ...
            if (stockOverviewResponse.getData().getQualityColor() == null
                    || stockOverviewResponse.getData().getQualityColor()
                            == SentimentColor.NEGATIVE) {
                return false;
            }

            if (stockOverviewResponse.getData().getValuationColor() == SentimentColor.NEGATIVE
            ...
            log.error(
                    "An error occured while getting research 360 insights {}",
                    stock.getNseSymbol(),
                    e);
        }
