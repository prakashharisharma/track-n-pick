...
public class SignalEvaluatorHelperService {
    ...
    public boolean isLowestAndLowMovingAverageDiffValid(
            Timeframe timeframe,
            StockPrice stockPrice,
            StockTechnicals stockTechnicals,
            MAInteractionType maInteractionType,
        ...
        // MarketCapCategory.classify(fundamentalResearchService.marketCap(stockTechnicals.getStock()));
        ...
        MovingAverageResult lowMovingAverageResult =
                MovingAverageUtil.getMovingAverage(
                        MovingAverageLength.LOW, timeframe, stockTechnicals, sortByValue);

        double lowestToLowDiff =
                formulaService.calculateAbsChangePercentage(
                        lowestMovingAverageResult.getPrevValue(),
                        lowMovingAverageResult.getPrevValue());

        boolean result = lowestToLowDiff >= threshold;

        evaluationLogService.add(
                stockPrice,
                result ? EvaluationLog.Type.POSITIVE : EvaluationLog.Type.NEUTRAL,
                StringUtils.format(
                        "{} Lowest to Low MA Diff:{} (> {}) → {}",
                        timeframe.name(),
                        lowestToLowDiff,
                        threshold,
                        result));
    ...
    public boolean isHigherTimeframeConfirmed(
        ...
        StockPrice stockPriceHigherTimeframe =
        ...
        double ma200 =
                MovingAverageUtil.getMovingAverage200(
                        stockPriceHigherTimeframe.getTimeframe(), stockTechnicalsHigherTimeframe);

        if (ma200 > 0 && stockPriceHigherTimeframe.getClose() < ma200) {
            return false;
        }
        ...
        if (ma200 == 0 && ma100 > 0 && stockPriceHigherTimeframe.getClose() < ma100) {
            return false;
        }
        ...
        if (ma200 == 0 && ma100 == 0 && ma50 > 0 && stockPriceHigherTimeframe.getClose() < ma50) {
            evaluationLogService.add(
                    stockTechnicals,
                    EvaluationLog.Type.NEUTRAL,
        ...
        double higherTimeframeClose = stockPriceHigherTimeframe.getClose();

        boolean isCloseAboveMA =
                stockPriceHigherTimeframe.getTimeframe() == Timeframe.WEEKLY
