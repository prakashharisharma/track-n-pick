...
@Service
@Slf4j
public class MovingAverageConvergenceDivergenceServiceImpl
        implements MovingAverageConvergenceDivergenceService {
    ...
    public List<MovingAverageConvergenceDivergence> calculate(List<OHLCV> ohlcvList) {
        ...
        for (int i = 0; i < ohlcvList.size(); i++) {
            ...
            if (i > longPeriod + signalPeriod - 2) {
                double ema =
                        formulaService.calculateEMA(
                                macdList.get(i), signalList.get(i - 1), signalPeriod);
    ...
    private List<MovingAverageConvergenceDivergence> mapResult(
            List<Double> ema12List,
            List<Double> ema26List,
            List<Double> macdList,
            List<Double> signalList) {
        List<MovingAverageConvergenceDivergence> movingAverageConvergenceDivergenceList =
                new ArrayList<>();
        for (int i = 0; i < macdList.size(); i++) {
            movingAverageConvergenceDivergenceList.add(
                    new MovingAverageConvergenceDivergence(
                            ema12List.get(i),
                            ema26List.get(i),
                            miscUtil.formatDouble(macdList.get(i), "00"),
                            miscUtil.formatDouble(signalList.get(i), "00")));
        }
