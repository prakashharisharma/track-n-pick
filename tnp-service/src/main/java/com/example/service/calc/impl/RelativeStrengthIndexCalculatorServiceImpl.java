...
@Service
@Slf4j
public class RelativeStrengthIndexCalculatorServiceImpl
        implements RelativeStrengthIndexCalculatorService {
    ...
    public List<RelativeStrengthIndex> calculate(List<OHLCV> ohlcvList) {
        ...
        for (int i = 0; i < ohlcvList.size(); i++) {
            ...
            if (i > 0) {
                ...
                // double upMove = (ohlcvList.get(i).getClose() - ohlcvList.get(i-1).getClose() ) >
                // 0 ? (ohlcvList.get(i).getClose() - ohlcvList.get(i-1).getClose() ) : 0;
                // double downMove = (ohlcvList.get(i-1).getClose() - ohlcvList.get(i).getClose() )
                // > 0 ? (ohlcvList.get(i-1).getClose() - ohlcvList.get(i).getClose() ) : 0;
                double upMove =
                        this.calculateUpMove(
                                ohlcvList.get(i).getClose(), ohlcvList.get(i - 1).getClose());
                double downMove =
                        this.calculateDownMove(
                                ohlcvList.get(i).getClose(), ohlcvList.get(i - 1).getClose());
                ...
                if (i == 14) {
                    ...
                    double smoothedAvgUpMove =
                            formulaService.calculateSmoothedMovingAverage(
                                    avgUps.get(i - 1), upMoves.get(i), 14);

                    double smoothedAvgDownMove =
                            formulaService.calculateSmoothedMovingAverage(
                                    avgDowns.get(i - 1), downMoves.get(i), 14);
        ...
        // return new RelativeStrengthIndex( avgUps.get(resultIndex),
        // avgDowns.get(resultIndex),rsList.get(resultIndex), rsiList.get(resultIndex) );
    }
    ...
    public RelativeStrengthIndex calculate(
        ...
        double upMove =
                this.calculateUpMove(ohlcvList.get(1).getClose(), ohlcvList.get(0).getClose());
        double downMove =
                this.calculateDownMove(ohlcvList.get(1).getClose(), ohlcvList.get(0).getClose());
        double avgUpMove =
                formulaService.calculateSmoothedMovingAverage(
                        prevRelativeStrengthIndex.getAvgUpMove(), upMove, 14);
        double avgDownMove =
                formulaService.calculateSmoothedMovingAverage(
                        prevRelativeStrengthIndex.getAvgDownMove(), downMove, 14);
        double rs = formulaService.calculateRs(avgUpMove, avgDownMove);
        double rsi = formulaService.calculateRsi(rs);
        return new RelativeStrengthIndex(
                avgUpMove, avgDownMove, rs, miscUtil.formatDouble(rsi, "00"));
    }

    private List<RelativeStrengthIndex> mapResult(
            List<Double> avgUps, List<Double> avgDowns, List<Double> rsList, List<Double> rsiList) {
        ...
        for (int i = 0; i < avgUps.size(); i++) {
            averageDirectionalIndexList.add(
                    new RelativeStrengthIndex(
                            ...
                            avgDowns.get(i),
                            rsList.get(i),
                            miscUtil.formatDouble(rsiList.get(i), "00")));
        }
