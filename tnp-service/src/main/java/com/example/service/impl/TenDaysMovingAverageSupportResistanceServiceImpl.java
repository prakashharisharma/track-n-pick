...
@Service
public class TenDaysMovingAverageSupportResistanceServiceImpl implements TenDaysMovingAverageSupportResistanceService {
    ...
    public boolean isBreakout(Timeframe timeframe, StockPrice stockPrice, StockTechnicals stockTechnicals) {
        ...
        //if(candleStickService.isHigherHigh(stockPrice) && candleStickService.range(stockPrice) > CandleStickService.MIN_RANGE) {
            //Breakout EMA20
            if (breakoutConfirmationService.isBullishConfirmation(timeframe, stockPrice, stockTechnicals, ema10) && breakoutService.isBreakOut(prevClose, prevEma10, close, ema10)) {
                return Boolean.TRUE;
            }
       // }
    ...
    public boolean isNearSupport(Timeframe timeframe, StockPrice stockPrice, StockTechnicals stockTechnicals) {
        ...
        //else if(candleStickService.isPrevLowerHigh(stockPrice) && candleStickService.isPrevLowerLow(stockPrice)) {
            // Near Support EMA 20
            if (supportResistanceConfirmationService.isSupportConfirmed(timeframe, stockPrice, stockTechnicals, ema10) && supportResistanceService.isNearSupport(prevOpen, prevHigh, prevLow, prevClose, prevEma10)) {
                return Boolean.TRUE;
            }
        //}
    ...
    public boolean isBreakdown(Timeframe timeframe, StockPrice stockPrice, StockTechnicals stockTechnicals) {
       ...
       // if(candleStickService.isLowerLow(stockPrice) && candleStickService.range(stockPrice) > CandleStickService.MIN_RANGE){
            //Breakdown EMA20
            if (breakoutConfirmationService.isBearishConfirmation(timeframe, stockPrice, stockTechnicals, ema10) && breakoutService.isBreakDown(prevClose, prevEma10, close, ema10)) {
                return Boolean.TRUE;
            }
       // }
    ...
    public boolean isNearResistance(Timeframe timeframe, StockPrice stockPrice, StockTechnicals stockTechnicals) {
       ...
       // else if(candleStickService.isPrevHigherHigh(stockPrice) && candleStickService.isPrevHigherLow(stockPrice)){
            // Near Support EMA 20
            if (supportResistanceConfirmationService.isResistanceConfirmed(timeframe, stockPrice, stockTechnicals, ema10) && supportResistanceService.isNearResistance(prevOpen, prevHigh, prevLow, prevClose, prevEma10)) {
                return Boolean.TRUE;
            }
       // }
