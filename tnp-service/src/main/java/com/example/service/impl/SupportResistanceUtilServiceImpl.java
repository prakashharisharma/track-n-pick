...
public class SupportResistanceUtilServiceImpl implements SupportResistanceUtilService {
    ...
    public boolean isNearSupport(
        ...
        if (close > average) {
            if (formulaService.inRange(low, high, average)
                    || formulaService.isEpsilonEqual(
                            low, average, FibonacciRatio.RATIO_161_8 * 10)) {
    ...
    private boolean isLowNearAverage(
            Stock stock,
            double open,
            double high,
            double low,
            double close,
            double average,
            double immediateLowAverage,
            boolean isLongEma) {

        if (formulaService.applyPercentChange(immediateLowAverage, FibonacciRatio.RATIO_38_2 * -100)
                >= average) {
            if (close >= average) {
                if (formulaService.inRange(low, high, average)
                        || formulaService.isEpsilonEqual(
                                low, average, FibonacciRatio.RATIO_161_8)) {
                    return Boolean.TRUE;
                } else if (isLongEma
                        && formulaService.calculateChangePercentage(average, low)
                                <= -1 * FibonacciRatio.RATIO_161_8 * 100) {
                    return Boolean.TRUE;
                }
            }
        }
    ...
    public boolean isNearResistance(
        ...
        if (close < average) {
            if (formulaService.inRange(low, high, average)
                    || formulaService.isEpsilonEqual(
                            high, average, FibonacciRatio.RATIO_161_8 * 10)) {
    ...
    private boolean isHighNearAverage(
            Stock stock,
            double open,
            double high,
            double low,
        ...
        if (formulaService.applyPercentChange(immediateLowAverage, FibonacciRatio.RATIO_38_2 * 100)
            ...
            if (close <= average) {
                if (formulaService.inRange(low, high, average)
                        || formulaService.isEpsilonEqual(
                                high, average, FibonacciRatio.RATIO_161_8)) {
                    return Boolean.TRUE;
                }
            } else if (!isLongEma
                    && formulaService.calculateChangePercentage(average, high)
                            >= FibonacciRatio.RATIO_161_8 * 100) {
                return Boolean.TRUE;
            }
        }
