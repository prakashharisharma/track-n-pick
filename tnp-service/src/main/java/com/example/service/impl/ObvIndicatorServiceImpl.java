    ...
    private boolean isBullishCurrentSession(StockTechnicals stockTechnicals) {
        if (stockTechnicals.getObv() < stockTechnicals.getObvAvg()) {
            if (this.isObvIncreasing(stockTechnicals) && this.isObvAvgDecreasing(stockTechnicals)) {
                return Boolean.TRUE;
            } else if (this.isObvIncreasing(stockTechnicals)
                    && this.isObvAvgIncreasing(stockTechnicals)) {
                return Boolean.TRUE;
            }
        } else {
            ...
            return CrossOverUtil.isFastCrossesAboveSlow(
                    stockTechnicals.getPrevObv(),
                    stockTechnicals.getPrevObvAvg(),
                    stockTechnicals.getObv(),
                    stockTechnicals.getObvAvg());
        }
    ...
    private boolean isBearishCurrentSession(StockTechnicals stockTechnicals) {
        ...
        if (stockTechnicals.getObv() > stockTechnicals.getObvAvg()) {
            if (this.isObvDecreasing(stockTechnicals) && this.isObvAvgIncreasing(stockTechnicals)) {
                return Boolean.TRUE;
            } else if (this.isObvDecreasing(stockTechnicals)
                    && this.isObvAvgDecreasing(stockTechnicals)) {
                return Boolean.TRUE;
            }
        } else {
