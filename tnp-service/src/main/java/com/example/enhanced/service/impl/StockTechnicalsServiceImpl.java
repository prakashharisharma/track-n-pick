package com.example.enhanced.service.impl;

import com.example.enhanced.model.stocks.*;
import com.example.enhanced.repo.stocks.StockTechnicalsRepository;
...
public class StockTechnicalsServiceImpl implements StockTechnicalsService {
    ...
    private static final Map<Timeframe, Supplier<StockTechnicals>> STOCK_TECHNICALS_CREATORS = Map.of(
    ...
    @Override
    public StockTechnicals create(Long stockId, Timeframe timeframe, Double ema5, Double ema10, Double ema20, Double ema50, Double ema100, Double ema200,Double sma5, Double sma10, Double sma20, Double sma50, Double sma100, Double sma200, Double rsi, Double macd, Double signal, Long obv, Long obvAvg, Long volume, Long volumeAvg5, Long volumeAvg10, Long volumeAvg20, Double adx, Double plusDi, Double minusDi,Double atr, LocalDate sessionDate) {
        Stock stock = getStockById(stockId);
        return this.create(stock, timeframe, ema5, ema10, ema20, ema50, ema100, ema200,sma5, sma10, sma20, sma50, sma100, sma200, rsi, macd, signal,obv, obvAvg, volume, volumeAvg5, volumeAvg10, volumeAvg20, adx, plusDi, minusDi, atr, sessionDate);
    }

    @Override
    public StockTechnicals create(Stock stock, Timeframe timeframe, Double ema5, Double ema10, Double ema20, Double ema50, Double ema100, Double ema200,Double sma5, Double sma10, Double sma20, Double sma50, Double sma100, Double sma200, Double rsi, Double macd, Double signal, Long obv, Long obvAvg, Long volume, Long volumeAvg5, Long volumeAvg10, Long volumeAvg20, Double adx, Double plusDi, Double minusDi, Double atr, LocalDate sessionDate) {
        Optional<StockTechnicals> existingStockTechnicals = stockTechnicalsRepository.findByStockIdAndTimeframe(stock.getStockId(), timeframe);

        // If a record with the same sessionDate exists, do nothing (return existing record)
        if (existingStockTechnicals.isPresent()) {
            log.info("Skipping creation. A StockTechnicals record already exists for stockId: {}, timeframe: {}",
                    stock.getStockId(), timeframe);
            return existingStockTechnicals.get();
        }


        StockTechnicals newStockTechnicals = STOCK_TECHNICALS_CREATORS
    ...
    @Override
    public StockTechnicals update(Long stockId, Timeframe timeframe, Double ema5, Double ema10, Double ema20, Double ema50, Double ema100, Double ema200,Double sma5, Double sma10, Double sma20, Double sma50, Double sma100, Double sma200, Double rsi, Double macd, Double signal, Long obv, Long obvAvg, Long volume, Long volumeAvg5, Long volumeAvg10, Long volumeAvg20, Double adx, Double plusDi, Double minusDi, Double atr, LocalDate sessionDate) {
        Stock stock = getStockById(stockId);
        return this.update(stock, timeframe, ema5, ema10, ema20, ema50, ema100, ema200,sma5, sma10, sma20, sma50, sma100, sma200, rsi, macd, signal,obv, obvAvg, volume, volumeAvg5, volumeAvg10, volumeAvg20, adx, plusDi, minusDi, atr, sessionDate);
    }

    @Override
    public StockTechnicals update(Stock stock, Timeframe timeframe, Double ema5, Double ema10, Double ema20, Double ema50, Double ema100, Double ema200,Double sma5, Double sma10, Double sma20, Double sma50, Double sma100, Double sma200, Double rsi, Double macd, Double signal, Long obv, Long obvAvg, Long volume, Long volumeAvg5, Long volumeAvg10, Long volumeAvg20, Double adx, Double plusDi, Double minusDi, Double atr, LocalDate sessionDate) {
        StockTechnicals stockTechnicals = stockTechnicalsRepository.findByStockIdAndTimeframe(stock.getStockId(), timeframe)
                .orElseThrow(() -> new EntityNotFoundException("StockTechnicals not found for stockId: " + stock.getStockId() + " and timeframe: " + timeframe));
        ...
        if (!sessionDate.isAfter(stockTechnicals.getSessionDate())) {
            log.info("Skipping update. Provided sessionDate: {} is not after existing sessionDate: {} for stockId: {}, timeframe: {}",
                    sessionDate, stockTechnicals.getSessionDate(), stock.getStockId(), timeframe);
    ...
    @Override
    public StockTechnicals createOrUpdate(Long stockId, Timeframe timeframe, Double ema5, Double ema10, Double ema20, Double ema50, Double ema100, Double ema200,Double sma5, Double sma10, Double sma20, Double sma50, Double sma100, Double sma200, Double rsi, Double macd, Double signal, Long obv, Long obvAvg, Long volume, Long volumeAvg5, Long volumeAvg10, Long volumeAvg20, Double adx, Double plusDi, Double minusDi, Double atr, LocalDate sessionDate) {
        ...
        if (existingStockTechnicals != null) {
            ...
            if (sessionDate.equals(stockTechnicals.getSessionDate())) {
                log.info("stockTechnicals already exists for stockId: {}, timeframe: {}, sessionDate: {}. No action taken.", stockId, timeframe, sessionDate);
                return stockTechnicals;
            }

            // If sessionDate is after, update it
            if (sessionDate.isAfter(stockTechnicals.getSessionDate())) {
                return update(stockId, timeframe, ema5, ema10, ema20, ema50, ema100, ema200,sma5, sma10, sma20, sma50, sma100, sma200, rsi, macd, signal,obv, obvAvg, volume, volumeAvg5, volumeAvg10, volumeAvg20, adx, plusDi, minusDi, atr, sessionDate);
            }

            // If sessionDate is before, do nothing
            log.info("Skipping update. Provided sessionDate: {} is before existing sessionDate: {} for stockId: {}, timeframe: {}", sessionDate, stockTechnicals.getSessionDate(), stockId, timeframe);
            return stockTechnicals;
        }

        // No existing record, create a new one
        return create(stockId, timeframe, ema5, ema10, ema20, ema50, ema100, ema200,sma5, sma10, sma20, sma50, sma100, sma200, rsi, macd, signal,obv, obvAvg, volume, volumeAvg5, volumeAvg10, volumeAvg20, adx, plusDi, minusDi, atr, sessionDate);

    }

    @Override
    public StockTechnicals createOrUpdate(Stock stock, Timeframe timeframe, Double ema5, Double ema10, Double ema20, Double ema50, Double ema100, Double ema200,Double sma5, Double sma10, Double sma20, Double sma50, Double sma100, Double sma200, Double rsi, Double macd, Double signal, Long obv, Long obvAvg, Long volume, Long volumeAvg5, Long volumeAvg10, Long volumeAvg20, Double adx, Double plusDi, Double minusDi, Double atr, LocalDate sessionDate) {
        ...
        if (existingStockTechnicals !=  null) {
            ...
            // If sessionDate is after, update it
            if (sessionDate.isAfter(stockTechnicals.getSessionDate())) {
                return update(stock, timeframe, ema5, ema10, ema20, ema50, ema100, ema200,sma5, sma10, sma20, sma50, sma100, sma200, rsi, macd, signal,obv, obvAvg, volume, volumeAvg5, volumeAvg10, volumeAvg20, adx, plusDi, minusDi, atr, sessionDate);
            }

            // If sessionDate is before, do nothing
            log.info("Skipping update. Provided sessionDate: {} is before existing sessionDate: {} for stockId: {}, timeframe: {}", sessionDate, stockTechnicals.getSessionDate(), stock.getStockId(), timeframe);
            return stockTechnicals;
        }

        // No existing record, create a new one
        return create(stock, timeframe, ema5, ema10, ema20, ema50, ema100, ema200,sma5, sma10, sma20, sma50, sma100, sma200, rsi, macd, signal,obv, obvAvg, volume, volumeAvg5, volumeAvg10, volumeAvg20, adx, plusDi, minusDi, atr, sessionDate);

    }

    @Override
    public StockTechnicals get(Long stockId, Timeframe timeframe) {
        return stockTechnicalsRepository.findByStockIdAndTimeframe(stockId, timeframe).orElseThrow(() -> new EntityNotFoundException("StockTechnicals not found for stockId: " + stockId + " and timeframe: " + timeframe));
    }

    @Override
    public StockTechnicals get(Stock stock, Timeframe timeframe) {
        return stockTechnicalsRepository.findByStockIdAndTimeframe(stock.getStockId(), timeframe).orElse(null);
    }
